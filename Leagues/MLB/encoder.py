...
def preprocess(data: pd.DataFrame) -> pd.DataFrame:
    ...
    data['1B-rate'] = (data['H'] - data['2B'] -
                       data['3B'] - data['HR']) / data['AB']

    # Calculate composite metrics
    data['IsoPower'] = (data['H'] + (data['2B'] * 2) +
                        (data['3B'] * 3) + (data['HR'] * 4)) / data['AB']
    data['OBP+SLG'] = ((data['H'] + data['BB']) / (data['AB'] + data['BB'])) + (
        (data['H'] + (data['2B'] * 2) + (data['3B'] * 3) + (data['HR'] * 4)) / (data['AB']))
...
if __name__ == "__main__":
    # Load the data
    data = pd.read_excel("Leagues/MLB/data/2024/5gw.xlsx", header=0)
    # Preprocess the data
    # Drop rows with 0 in any denominator to avoid inf/NaN
    data = data[(data['PA'] > 0) & (data['AB'] > 0) & (data['G'] > 0)]
    ...
    autoencoder.fit(features_scaled, features_scaled, epochs=100,
                    batch_size=32, shuffle=True, validation_split=0.2)

    # --- Encode the data ---
    encoded_ouput = autoencoder.encoder.predict(features_scaled)
    encoded_df = pd.DataFrame(encoded_ouput, columns=[
                              f'encoded_{i}' for i in range(encoded_ouput.shape[1])])

    # --- Combine with meta data ---
    final_encoded_df = pd.concat([meta_data, encoded_df], axis=1)

    # Save the data
    final_encoded_df.to_csv("Leagues/MLB/data/encoded_5game_spans.csv", index=False)
